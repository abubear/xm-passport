...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body: any = { display_name: displayName, auth_provider: method };
      if (method === 'email') {
        body.email = email;
    ...
    fetch('/api/auth/register', {
      ...
      .then((data) => {
        if (data.user) {
          ...
          setError(data.error || `${provider} sign-up failed`);
        }
      })
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center p-6 safe-top safe-bottom">
      ...
      <h1 className="text-2xl font-bold mb-2">Create Passport</h1>
      <p className="text-gray-500 mb-6">Join the XM collector ecosystem</p>
        ...
        {method === 'email' ? (
          ...
          <div>
            ...
            <input
              ...
              autoComplete="tel"
            />
            <p className="text-[10px] text-gray-600 mt-1">We will send a verification code</p>
          </div>
        )}
      ...
      <div className="space-y-3">
        ...
        >
          <WechatIcon /> Sign up with WeChat
        </button>
      </div>

      <p className="mt-6 text-center text-sm text-gray-500">
        Already have a Passport?{' '}
        <a href="/login" className="text-xm-gold">Sign in</a>
      </p>
    </div>
  );
}
