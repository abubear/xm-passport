...
export default async function RedeemPage() {
  ...
  const etickets = db.prepare(`
    SELECT * FROM etickets WHERE owner_id = ? AND status = 'active' ORDER BY created_at DESC
  `).all(payload.id) as ETicket[];

  const redeemedTickets = db.prepare(`
    SELECT * FROM etickets WHERE owner_id = ? AND status = 'redeemed' ORDER BY redemption_date DESC
  `).all(payload.id) as ETicket[];
